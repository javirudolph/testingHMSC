...
#outsfolderpath <- "outputs/20190409Out/"
outsfolderpath <- "outputs/20190416out/"
...
csvFiles <- list.files(outsfolderpath)
...
for(i in 1:7){
  data <- read.csv(paste0(outsfolderpath, sppFiles[i])) 
...
for(i in 1:7){
  data <- read.csv(paste0(outsfolderpath, sitesFiles[i])) %>% 
    mutate(E = rep(E, 5), 
           Edev = abs(E-0.5))
