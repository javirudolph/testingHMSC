...
WR %>%
  ...
  geom_encircle(data = WR %>% filter(dispersal == 0.01),
                aes(group = dispersal), size = 0.7, color = NA, fill = "#FDE4A6FF",alpha = 0.6, expand = 0.02) +
  ...
  guides(size = guide_legend(title = expression(R^2), order = 2, nrow = 1, label.position = "bottom"),
         fill = guide_colorbar(title = "Niche \noptima", order = 1, barheight = 0.5),
         color = guide_colorbar(title = "Environmental\ndeviation", order = 3, barheight = 0.5))

ggsave(paste0(tiff_path, "Figure4.tiff"), dpi = 600, width = 6, height = 4.5)
