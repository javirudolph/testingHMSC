...
As an example, using the aggregated XY coordinates calculated in the 'Patch locations and environmental variables' section, for `r N` patches and our initial occupancy matrix with `r R` species ,we can see the connectivity between patches, and calculate the contribution of immigration from each species to each patch.    
...
## Environmental filtering  
In the section above, it is refered to the effect of environmental filtering on the probability of establishing a viable local population. The effect of the environment is given by a product of the establishment performance over all environmental variables $E_n$:
\begin{equation}
\tag{A3}
...
fig3aplot_spp + theme(legend.position = "none") -> Sup1A

leg <- get_legend(fig3aplot_spp)
as_ggplot(leg) -> legendSup1A
...
fig3aplot_sites + theme(legend.position = "none") -> Sup1B

leg <- get_legend(fig3aplot_sites)
as_ggplot(leg) -> legendSup1B


grid.arrange(Sup1A, Sup1B, ncol = 2)
grid.arrange(legendSup1A, legendSup1B, ncol = 2)

```


Supplementary Figure 2.  In which we change dispersal only, α was 0.01 for 1/3 of species, 0.05 for 1/3 of species, and 0.1 for the other 1/3 of species.

Appendix ## – Calculation of the coefficient of determination (R^2)
Classic coefficient of determination (R^2)

The coefficient of determination that was partitioned in the variation partitioning analysis (Appendix XX) is calculated for species j as 

	R_j^2= 1-  (∑_(i=1)^n▒(y_ij-y ̂_ij )^2 )/(∑_(i=1)^n▒(y_ij-y ̅_ij )^2 )	eq A1

where y_ij is the data associated with species j (out of p species) at site i (out of n sites), y ̂_ij is the model associated to species j at site i and y ̅_j is the average of the data for species j across all sites. 
...
Although having an R_j^2 for each species j can be highly informative, it is sometimes useful to also have a single R^2 for the entire community. This is obtained by averaging all R_j^2 
	...
	(_^C)R^2 =(∑_(j=1)^p▒R_j^2 )/p	eq A2
where the (_^C)R^2  is the community-level R^2.
...
In the paper, we use the contribution of each site to R_j^2 to present how each site contributes differently to the environment, space and co-distribution for the community. The calculation of the site i contribution to the R_j^2, is calculated as 
	...
	R_ij^2=1/n  -  (y_ij-y ̂_ij )^2/(y_ij-y ̅_ij )^2 .	eq A3

When studying equation A2, a few considerations can be made. The first part of the equation where the 1 of the classic R_j^2 is divided by n is included to make sure that if we sum all R_ij^2 across all sites for species j, the resulting value is R_j^2 and not some other value. 

More importantly, what can be noticed is that by calculating R_ij^2, the contribution of sites to each species R_j^2, we obtain a matrix that has the same dimension as the site by species matrix (an n×p matrix). Using this matrix, if we sum across all sites, we obtain the R_j^2. However, if we average across the species for site i we obtain the site’s contribution to the community-level (_^C)R^2  or (_^C)R_i^2 . 
The amount of variation expressed by the R_j^2, the (_^C)R^2 , the R_ij^2 or the (_^C)R_i^2   can all be partitioned in its environmental, spatial and co-distribution component following the procedure presented in (Appendix XX).

Pseudo coefficient of determination (pseudo-R^2)

There exist different variants of pseudo-R^2 that can be applied to specific types of data (e.g. Tjur’s D [Tjur 2009] is designed to presence-absence data) or more broadly (e.g. Efron’s pseudo-R^2 [Efron 1978]). Because all the different variants of pseudo-R^2 can be applied to a model with a single response variable (i.e. a single species), community-level pseudo-R^2 can be obtained following the approach mentioned above. However, this is not always possible when the interest is to calculate the sites’ contribution to a pseudo-R^2, especially for variants of pseudo-R^2 that do not have a mathematical structure similar to the classical R^2.
