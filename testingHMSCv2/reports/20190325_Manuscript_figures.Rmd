...
species_plot <- function(data, scenario, colorVar = NULL){
  data %>% 
    ...
    guides(color = guide_legend(title = paste(colorVar)), 
...
for(i in 1:4){
  ...
  figData <- get_species_data(folderpath, scenario)
  plot <- species_plot(figData, scenario, colorVar = "iteration")
  print(plot)
}
...
```{r}
sppFig3a <- get_species_data(folderpath, "FIG3A") %>% 
  left_join(., get_fig3_params(folderpath, "Fig3A"))

species_plot(sppFig3a, scenario = "FIG3A", "intercol") +
  scale_color_viridis_d()
```

```{r}
sppFig3b <- get_species_data(folderpath, "FIG3B") %>% 
  left_join(., get_fig3_params(folderpath, "Fig3B"))

species_plot(sppFig3b, scenario = "FIG3B", "dispersal")
```


```{r}
sppFig3c <- get_species_data(folderpath, "FIG3C") %>% 
  left_join(., get_fig3_params(folderpath, "Fig3C"))

species_plot(sppFig3c, scenario = "FIG3C", "iteration")
```

# Site figures as density

```{r}
sitesFig2a <- get_sites_data(folderpath, "FIG2A")
species_plot(sitesFig2a, "FIG2A", colorVar = "iteration")
```

## Density plot instead
```{r}
sites_dens_plot(sitesFig2a, scenario = "FIG2A")
