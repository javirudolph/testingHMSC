...
```{r}
path_without_interactions <- "outputs/20190325-breadth_w_o_interactions/"
...
for(i in 1:length(values)){
  ...
  breadth <- values[i]
  figData <- get_species_data(path_with_interactions, scenario)
  plot <- species_plot(figData, scenario = paste0("Breadth = ", breadth), colorVar = "iteration")
}
```
...
Testing niche breadth without species interactions:
```{r, eval = FALSE}
path_with_interactions <- "outputs/20190325-breadth_with_interactions/"
```

```{r, eval = FALSE, fig.width=14, fig.height=4}
for(i in 1:length(values)){
  scenario <- paste0("scenario", i)
  breadth <- values[i]
  figData <- get_species_data(path_with_interactions, scenario)
